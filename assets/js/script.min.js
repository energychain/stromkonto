...
function resolvAddress(address) {    
    if(address=="0x445c1e284c15a50a69fe7d6dcd9fba3b938b52bb") return "Eigenerzeugung";
    if(address=="0xdcbab2b1c28b6f57c367f4eff0f5c7893400cc4d") return "Grünstrombonus";
    if(address=="0x961aa3954c3af05c975433f095e9567780a4d6d2") return "Epiktet Beteiligung - Rhein-Neckar";
    if(address=="0x41b7931ea8f072dcc77f1f682cd5a1273ff7dbe6") return "Epiktet Beteiligung - Saarland";
    ...
    CorrentlyWallet.default.Stromkonto($('#account').val()).then(function(l) { 
        CorrentlyWallet.default.CorrentlyAccount($('#account').val()).then(function(account) {
                ...
                if(typeof stromkonto_data.account.clearedGeneration != "undefined") {    
                    stromkonto_data.account.preGain=stromkonto_data.account.clearedGeneration;
                ...
                for(var i=0;i<stromkonto_data.account.txs.length;i++) {
                    stromkonto_data.account.txs[i].generation = Math.abs(stromkonto_data.account.txs[i].cori * ((stromkonto_data.account.txs[i].timeStamp-ts)/(86400000*365)));
                    stromkonto_data.account.txs[i].peer=resolvAddress(stromkonto_data.account.txs[i].contract);
                    stromkonto_data.account.txs[i].timeStamp=new Date(stromkonto_data.account.txs[i].timeStamp).toLocaleString();
                    console.log(stromkonto_data.account.txs[i].start,stromkonto_data.account.txs[i].cori);
                }
                ...
                $('#btn_tx').click(function() {toggle('tx-card','toggle_tx');});
                $('#btn_asset').click(function() {toggle('asset-card','toggle_asset');});
